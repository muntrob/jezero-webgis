...
var allPRTargetsWfs = new Vector({
  title: 'PR Targets [SELECTABLE]', // 'title' parameter is necessary to be able to make the layer (in-)visible
  //  visible: false,
  source: new VectorSource({
    wrapX: false
  })
...
var initPRTargets = function () {
  allPRTargetsWfs.getSource().clear();
  var queryLayer = '';
  var featureRequest = new WFS().writeGetFeature({
    // featureNS: 'https://maps.planet.fu-berlin.de/eqc-bin/wms?',
    featureNS: 'https://maps.planet.fu-berlin.de/eqc-bin/wfs?',
  ...
  fetch('https://maps.planet.fu-berlin.de/eqc-bin/wfs?', {
    ...
    var features = new GeoJSON().readFeatures(json);
    allPRTargetsWfs.getSource().addFeatures(features);
    console.dir(features.length + ' PR sequences retrieved.');
...
var refreshPRTargets = function () {
  pr_targets_lay.getSource().clear();
